    ...
    >
        <form class="space-y-6">
            ...
            <label for="product_colors">
                ...
                <div class="bg-gray-50 rounded-xl ring-1 ring-gray-300 p-2">
                    <label :for="'product_color_' + color" v-for="color in color_list" :key="color.id">
                        <input v-model="colors" type="checkbox" name="color" :value="color" :id="'product_color_' + color" required>
                        {{color}}
                    </label>
                </div>
            </label>
            <div>
                ...
                <div class="bg-gray-50 rounded-xl ring-1 ring-gray-300 p-2">
                    <label for="product_type_wall">
                        <input type="checkbox" name="type" id="product_type_wall" required>
                        Wall
                    </label>
                    <label for="product_type_porch">
                        <input type="checkbox" name="type" id="product_type_porch" required>
                        Porch
                    </label>
                    <label for="product_type_seasonal">
                        <input type="checkbox" name="type" id="product_type_seasonal" required>
                        Seasonal
                    </label>
                </div>
            </div>
...
export default {
    ...
    methods: {
        uploadImage(input) {
            if(input)   this.image = URL.createObjectURL(input.target.files[0])
        }
    }
}
</script>
