<template>
  <transition name="fade" v-on:after-enter="afterEnter" v-on:before-leave="afterLeave">
    <div
      class="top-0 bottom-0 left-0 right-0 fixed flex flex-col md:flex-row justify-end z-50 bg-gray-900 bg-opacity-10"
    >
      ...
      <transition name="slide">
        ...
        >
          <section class="w-full h-full pb-20 flex flex-col text-md md:text-xl relative">
            <div class="w-full h-96 flex-none bg-gray-50 flex items-center justify-center">
              <button
                aria-label="Close Modal"
                @click="close"
                class="z-50 h-6 w-6 absolute top-0 right-0 mt-2 mr-4 ring-1 ring-red-500 p-1 bg-red-200 text-red-500 rounded-full"
              >
                <svg
            ...
            <div class="absolute top-0 h-full flex flex-col w-full overflow-y-auto">
              ...
              >
                ...
                <h1 class="mt-2 font-bold text-3xl md:text-4xl truncate">
                  {{title}}
                </h1>
                <h3 class="mt-2 text-3xl md:text-4xl font-bold">${{price}}</h3>
...
export default {
  name: 'Modal',
  props: ['colors', 'title', 'price', 'id', 'editing', 'description', 'image'],
  data() {
    return {
      count: 1,
      secondary: false,
